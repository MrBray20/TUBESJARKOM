...
public class Server {
    ...
    private static void executeSQL(Connection connection) {
        String[] sqlStatements = {
                ...
                // ")",
                //
                // "ALTER TABLE `rooms` ADD CONSTRAINT `rooms_fk4` FOREIGN KEY (`own_room`)
                // REFERENCES `users`(`id_user`)",
                // "ALTER TABLE `message` ADD CONSTRAINT `message_fk1` FOREIGN KEY (`user_id`)
                // REFERENCES `users`(`id_user`)",
                // "ALTER TABLE `message` ADD CONSTRAINT `message_fk2` FOREIGN KEY (`room_id`)
                // REFERENCES `rooms`(`id_room`)",
                // "ALTER TABLE `room_members` ADD CONSTRAINT `room_members_fk0` FOREIGN KEY
                // (`user_id`) REFERENCES `users`(`id_user`)",
                // "ALTER TABLE `room_members` ADD CONSTRAINT `room_members_fk1` FOREIGN KEY
                // (`room_id`) REFERENCES `rooms`(`id_room`)",
                // "ALTER TABLE `friends` ADD CONSTRAINT `friends_fk1` FOREIGN KEY (`id_user1`)
                // REFERENCES `users`(`id_user`)",
                // "ALTER TABLE `friends` ADD CONSTRAINT `friends_fk2` FOREIGN KEY (`id_user2`)
                // REFERENCES `users`(`id_user`)"
        };
